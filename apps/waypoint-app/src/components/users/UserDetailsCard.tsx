...
export const UserDetailsCard: React.FC<UserDetailsCardProps> = ({
    ...
    (id: string) => {
      deleteUserDetails(id, {
        ...
        onError: (error) => {
          toast({
            title: 'Error',
            description: `Failed to delete user details: ${error.message}`,
          });
  ...
  return (
    <Card
      className={
        isDefault
          ? 'border border-indigo-200 shadow-md bg-indigo-50'
    ...
    >
      <CardHeader
        className={
          isDefault
            ? 'bg-indigo-100 py-4 px-5 border-b border-indigo-200 flex flex-row items-center justify-between'
            : 'bg-blue-50 py-3 px-4 border-b border-blue-100 flex flex-row items-center justify-between'
        }
      >
        <CardTitle
          className={
            isDefault
              ? 'text-lg text-indigo-800 flex items-center gap-2'
              : 'text-md text-gray-700 flex items-center gap-2'
          }
        >
          {isDefault && (
          ...
          <MapPin
            size={isDefault ? 20 : 16}
            className={isDefault ? 'text-indigo-600' : 'text-blue-600'}
          />
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  ...
                  these user details.
                  {isDefault && (
                    <span className="mt-2 text-rose-500 font-medium">
      ...
      <CardContent
        className={isDefault ? 'p-5 space-y-3' : 'p-4 space-y-2 text-sm'}
      >
        <div
          className={
            isDefault
        ...
        >
          <MapPin
            ...
            className={isDefault ? 'text-indigo-500' : 'text-blue-500'}
          />
          <span>
            {detail.addressLine1}, {detail.city}, {detail.state}{' '}
